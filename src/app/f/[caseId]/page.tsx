...
import { Callout, StatusChip } from "@/ui/bits";
import { fmtDate, fmtDays, fmtMoney, fmtWeekday } from "@/lib/format";
import { AlertList, CaseHead, OwnerCard, StageLedger, Timeline } from "./parts";
...
export default async function CaseFile({
  ...
  return (
    <Shell lang={lang} narrow>
      {locked ? (
        <Callout tone="ok" title="आवेदन लॉक हो गया">
          <p>
            पंजीकरण संख्या <span className="mono">{c.registrationNo}</span>. अंतिम प्रिंट, शुल्क रसीद
            और मार्कशीट{" "}
            <strong>{fmtWeekday(c.hardCopy.dueAt)}</strong> तक कॉलेज में जमा करें और रसीद लें।
          </p>
        </Callout>
      ) : null}

      <p className="eyebrow" style={{ marginTop: locked ? "var(--s5)" : 0 }}>
        {c.trackHi} · {c.cycleHi} · {c.id}
        {c.registrationNo ? ` · पंजीकरण ${c.registrationNo}` : ""}
      </p>
      <h1 style={{ marginTop: "var(--s3)" }}>आपकी फ़ाइल</h1>
      ...
      <div className="stack" style={{ ["--gap" as string]: "var(--s5)", marginTop: "var(--s5)" }}>
        ...
        {c.escalations.length > 0 ? (
          <Callout tone="warn" title="स्वतः भेजे गए अनुरोध">
            <ul>
            ...
            <p style={{ marginTop: "var(--s2)", fontSize: "var(--step-s)" }}>
              प्रतीक्षा की गिनती इससे शून्य नहीं होती। <Link href={`/shikayat/${c.id}`}>शिकायत का
              मसौदा देखें →</Link>
            </p>
          </Callout>
        ) : null}
        ...
        {c.flags.length > 0 ? (
          <section className="stack">
            ...
            <div className="stack" style={{ ["--gap" as string]: "var(--s3)" }}>
              {c.flags.map((f, i) => (
                <div className="sheet stack" key={`${f.code}-${i}`} style={{ ["--gap" as string]: "var(--s2)" }}>
                  <div className="row-between">
                    <strong>{f.reason?.hi ?? f.code}</strong>
                    <StatusChip tone={f.reason?.correctable ? "waiting" : "breach"}>
                      {f.reason?.correctable ? "सुधार विंडो में ठीक होगा" : "सुधार विंडो से ठीक नहीं होगा"}
                    </StatusChip>
                  </div>
        ...
        <section className="stack">
          ...
          <dl className="sheet" style={{ margin: 0 }}>
            ...
            <div className="datarow">
              <dt>उपस्थिति</dt>
              <dd>{c.attendancePercent === null ? "संस्थान ने दर्ज नहीं की" : `${c.attendancePercent}%`}</dd>
            </div>
            <div className="datarow">
              <dt>हार्ड कॉपी</dt>
              <dd>
